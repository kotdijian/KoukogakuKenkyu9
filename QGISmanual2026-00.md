...
#### ver.2025.00
 - 本マニュアルはQGIS 3.44 （LTR: 長期サポート版)に準拠しています
 - 現行のLTRは今年後半までサポートが継続されることが表明されています
 - QGISは2025年10月にメジャーアップデート版(QGIS 4.0)がリリースされています。メニューや操作方法に大きな変化はありませんが一部のプラグインなどが使用できなくなっています
...
#### [2026年度授業ページトップに戻る](https://kotdijian.github.io/KoukogakuKenkyu9/)
...
#### QGISの基本的な画面構成
1. メニューバー:画面最上部、文字のメニュー。全ての機能をここから選択できる
2. ツールバー:初期設定でメニューバーの下、メニュー項目の中にある各機能のボタン、表示を設定できる
3. ブラウザパネル:初期設定で左上、利用可能なソースが表示される
4. レイヤパネル:初期設定で左下、読み込んだデータを表示するレイヤが一覧表示される
5. マップキャンバス:アクティブな地図画面が表示される
- そのほか、必要に応じてサブパネルが開く
...
- 左下のレイヤパネルに指定した地図タイルのレイヤがあることを確認
- 国土地理院：地理院タイルについて　https://maps.gsi.go.jp/development/siyou.html
- 解説：https://qgis.mierune.co.jp/posts/howto_1_add_xyztiles

#### レイヤの基本操作

- レイヤの順序(上下)は画面上の表示の重ね合わせの順序
- 面的なデータ=ラスタレイヤの場合、表示順により下のレイヤは見えない
   - レイヤの透過度の設定による重ね合わせが可能
- 点・線・面などのデータ=ベクタレイヤの場合、オブジェクト(地物)と塗りの範囲以外は下のレイヤが見える
- レイヤ名の左端のチェックボックスのオン／オフでレイヤの表示・非表示を切り替えられる
- 複数のレイヤをまとめた「グループ」を作成可能 

#### 地図画面の基本操作
(上中央のツールボタン)
- **手のひらツール（パン)**：地図画面のスクロール、左クリックして移動
- **拡大ツール(ズームイン)**：地図画面の表示拡大、マウスホイールやピンチでも可能
- **縮小ツール(ズームアウト)**：地図画面の表示縮小、マウスホイールやピンチでも可能
- **全域表示ツール**：全体が見える大きさにリサイズ
