...
日本の行政界のベクターデータを簡単にRで描画させるためのjpndistrictパッケージは現在CRANに掲載されていないためGithubのリポジトリからダウンロードしてインストールします。以下のチャンクを実行して下記のような表示がでる場合は1を選択してください。
...
> 3: None                            
> 4: utf8     (1.2.5 -> 1.2.6) [CRAN]
> 5: evaluate (1.0.3 -> 1.0.4) [CRAN]
> 6: tibble   (3.2.1 -> 3.3.0) [CRAN]
> 7: curl     (6.2.2 -> 6.3.0) [CRAN]

```{r chunk1, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
# CRANにないパッケージインストールのためのremotesパッケージのインストール
if(!require("remotes")) install.packages('remotes', repos='http://cran.us.r-project.org') 
library(remotes)

install_github("uribo/jpndistrict")
library(jpndistrict)
```
...
title("TokyoSelf-AdministrationMap")
...
Tokyo.summary <- import("https://github.com/kotdijian/JASOSR/raw/master/13Tokyo/13TokyoSummary1.csv", setclass= "tbl_df", encoding = "UTF-8") # TokyoTotalに原データcsvを読み込み、エンコードの指定に注意
Tokyo.summary <- Tokyo.summary %>%
                 rename(city_code = LGC, Ya = Y)        　#列名変更
tokyo.bnd1$city_code <- as.integer(tokyo.bnd1$city_code)   #列のデータ型一致
Tokyo.summary <- left_join(tokyo.bnd1, dplyr::select(Tokyo.summary, -Name), by = "city_code")
...
ggplot(Tokyo.summary)+
  geom_sf(aes(fill = Sum)) + #遺跡数合計で色分け
  scale_fill_gradient(high = "#222222", low = "#dddddd") +
  labs(x = "Lon",
       y = "Lat",
       title = "TotalSiteNumbers"
       ) +
...
#地図描画
# library("scales")
# theme_set(theme_gray(base_family = "HiraKakuPro-W3")) # セッション中のプロットエリアの文字化けを防止

data %>%
ggplot()+
  geom_sf(data = Tokyo.summary,
          aes(fill = cut_number(Sum, n = 8))) + #遺跡数合計で色分け
...
#旧石器
tokyo.pal.na <- filter(Tokyo.summary, P == 0) #遺跡数0の自治体
p1a <- ggplot() +
  geom_sf(data = Tokyo.summary,
          aes(fill = P)) +
  ...
  geom_text(data = tokyo.pts,                      #遺跡数の表示設定
            ...
            color = "white",
            size = 3) +
  labs(x = "Lon", y = "Lat", title = "PalaeolithicSiteNumber"
       ) +
  geom_sf(data = tokyo, fill = NA, col = "black", cex = 1) +
  annotation_scale() +
  annotation_north_arrow(width = unit(1, "cm"), pad_x = unit(0.5, "cm"), pad_y = unit(1, "cm")) +
  theme_bw()
#縄文
tokyo.jom.na <- filter(Tokyo.summary, J == 0)
p2a <- ggplot() +
  geom_sf(data = Tokyo.summary,
          aes(fill = J)) +
  scale_fill_viridis_c(begin = 0.8, end = 0) +
  ...
  theme_bw()
#弥生
tokyo.yay.na <- filter(Tokyo.summary, Ya == 0)
p3a <- ggplot() +
  geom_sf(data = Tokyo.summary,
          aes(fill = Ya)) +
  scale_fill_viridis_c(begin = 0.8, end = 0) +
  ...
  theme_bw()
#古墳
tokyo.kof.na <- filter(Tokyo.summary, K == 0)
p4a <- ggplot() +
  geom_sf(data = Tokyo.summary,
          aes(fill = K)) +
  scale_fill_viridis_c(begin = 0.8, end = 0) +
...
leaflet(Tokyo.sf) %>%
  ...
  addCircleMarkers(radius = 3, color = "#09f", weight = 8,
                   popup = Tokyo.sf$Chronology, label = paste(Tokyo.sf$SiteName_original)
                   ) %>%
...
#対象遺跡の指定とデータの抽出
focal.site <- filter(Tokyo.sf,  SiteName_original == "下戸塚遺跡")
coord <- st_coordinates(focal.site)    #座標の抽出
focal.site <- cbind(focal.site, coord) #抽出した座標をデータに連結

#対象遺跡の所在自治体ポリゴンの抽出
city.bnd <- filter(tokyo.bnd1, city_code == focal.site$LGC)
...
Kofun.sf <- Tokyo.sf %>%
  ...
  mutate(MoundeType = 
             if_else(condition = str_detect(ArchaeologicalFeatures,"前方後円墳"), 
                     true = "前方後円墳",
                     false = 
                       if_else(condition = str_detect(ArchaeologicalFeatures,"前方後方墳"),
                               true = "前方後方墳",
                               false =
                                 if_else(condition = str_detect(ArchaeologicalFeatures,"上円下方墳"),
                                         true = "上円下方墳",
                                         false =
                                           if_else(condition = str_detect(ArchaeologicalFeatures,"円墳"),
                                                   true = "円墳",
                                                   false =
                                                     if_else(condition = str_detect(ArchaeologicalFeatures,"方墳"),
                                                             true = "方墳",
                                                             false = "NA")
                                                   )
                                         )
...
Kofun.sf$MoundType <- Kofun.sf$MoundType %>%
                 as.factor()  %>%                                                          #新たに追加した墳形列を因子型に変換
                 factor(levels = c("前方後円墳","前方後方墳","上円下方墳","円墳","方墳")) #因子型のデータの順列を指定
str(Kofun.sf)

colpal <- colorFactor(palette=c("orange","black","white","red","blue"), domain=Kofun.sf$MoundType) #墳形列データにカラーパレットを割り当て

leaflet(Kofun.sf) %>%
  addTiles("https://cyberjapandata.gsi.go.jp/xyz/relief/{z}/{x}/{y}.png",
           attribution = atr) %>%
  addCircleMarkers(radius = 4, weight = 2, color = ~colpal(MoundType),
                   popup = Kofun.sf$ArchaeologicalFeatures, label = paste(Kofun.sf$ArchaeologicalFeatures)
                   ) %>%
  setView(lng = 139.55, lat = 35.68944, zoom = 11) %>%
  addLegend(position='topright', pal=colpal, values=~MoundType) %>% #凡例を追加
  addMiniMap(position="bottomleft") %>% 
  addSimpleGraticule(interval = 0.25) %>%
  addScaleBar(position="bottomright",
              options = scaleBarOptions(imperial = FALSE)
...
tokyo.pal <- tokyo.sfz9 %>%
             filter(str_detect(Chronology, "旧石器時代")) #旧石器
tokyo.jom <- tokyo.sfz9 %>%
             filter(str_detect(Chronology, "縄文時代")) #縄文
tokyo.yay <- tokyo.sfz9 %>%
             filter(str_detect(Chronology, "弥生時代")) #弥生
tokyo.kof <- tokyo.sfz9 %>%
             filter(str_detect(Chronology, "古墳時代")) #古墳
tokyo.kod <- tokyo.sfz9 %>%
             filter(str_detect(Chronology, "奈良時代|平安時代")) #古代
tokyo.chu <- tokyo.sfz9 %>%
             filter(str_detect(Chronology, "中世")) #中世
```
...
p1 <- ggplot()+                                        #直接表示せず描画結果をp1に格納
       ...
       labs(x = "X", y = "Y",                     #軸のラベル:横軸(x)=経度、縦軸(y)=緯度
            title = "Palaeolithic"                        #グラフタイトル
            ) +
...
tokyo.pal.coord <- cbind(dplyr::select(tokyo.pal, JASID, SiteName_original), coord) #抽出した座標を元データに連結(JASID,遺跡名のみ)
tokyo.pal.cent <- data_frame(Age = "P",
...
```{r chunk32, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
#座標データの整備2
coord <- st_coordinates(tokyo.jom)
tokyo.jom.coord <- cbind(dplyr::select(tokyo.jom, JASID, SiteName_original), coord)
tokyo.jom.cent <- data_frame(Age = "J",
                             X = mean(tokyo.jom.coord$X),
                             Y = mean(tokyo.jom.coord$Y))

coord <- st_coordinates(tokyo.yay)
tokyo.yay.coord <- cbind(dplyr::select(tokyo.yay, JASID, SiteName_original), coord)
tokyo.yay.cent <- data_frame(Age = "Ya",
                             X = mean(tokyo.yay.coord$X),
                             Y = mean(tokyo.yay.coord$Y))

coord <- st_coordinates(tokyo.kof)
tokyo.kof.coord <- cbind(dplyr::select(tokyo.kof, JASID, SiteName_original), coord)
tokyo.kof.cent <- data_frame(Age = "K",
...
plot_grid(p1c, p2c, p3c, p4c)

#重心データを連結
tokyo.cent <- rbind(tokyo.pal.cent, tokyo.jom.cent, tokyo.yay.cent, tokyo.kof.cent)
tokyo.cent$Age <- as_factor(tokyo.cent$Age)
...
##グリッドの作成と遺跡数の集計
```{r chunk33, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
...
p1g <- ggplot(grd.tokyo.pal) +
        ...
        labs(x = "X", y = "Y", title = "DensityOfPalaeolithicSites(2sqKm)") +
        xlim(-82729.7502, 9073.9070) +
        ylim(-57316.0187, -8857.3635) +
        geom_sf(data = tokyo.z9, fill = NA, col = "black", cex = 1) +
        annotation_scale() +
